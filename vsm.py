...
#%% XI PERPENDICULAR
files_P=os.listdir(os.path.join(os.getcwd(),'Perpendicular'))
files_P.sort()
lim_ajuste_P=10
...
H_aux_P= np.linspace(-50,50,1000)
campo_lineal_P=[]
m_interp_P=[]
...
for f in files_P:
    ...
    m_correg_norm_P.append(m_norm)
    campos_P.append(np.array(H))
    fname_P.append(f.split('_')[-1].split('.')[0])
    ...
    indx_H = np.nonzero(H_aux>=0)[0][0]
    indx_M = np.nonzero(m_aux>=0)[0][0]
    Hc_P.append(-H_aux[indx_M])
    Mr_P.append(m_aux[indx_H])
    print('*'*50)
    print(f.split('_')[0], f.split('_')[-1])
    print('Susceptibilidad =',f'{chi:.3e}','+/-',f'{err_m:.3e}')
    print(f'Mag Remanente = {m_aux[indx_H]:.3e}')
    ...
    #plt.show()
#%% XI Axial
files_A=os.listdir(os.path.join(os.getcwd(),'Axial'))
lim_ajuste_A=10
...
H_aux_A= np.linspace(-50,50,1000)
campo_lineal_A=[]
m_interp_A=[]
...
for f in files_A:
    ...
    m_correg_norm_A.append(m_norm)
    campos_A.append(np.array(H))
    fname_A.append(f.split('_')[-1].split('.')[0])
    ...
    indx_H = np.nonzero(H_aux>=0)[0][0]
    indx_M = np.nonzero(m_aux>=0)[0][0]
    Hc_A.append(-H_aux[indx_M])
    Mr_A.append(m_aux[indx_H])
    print('*'*50)
    print(f.split('_')[0], f.split('_')[-1])
    print('Susceptibilidad =',f'{chi:.3e}','+/-',f'{err_m:.3e}')
    print(f'Mag Remanente = {m_aux[indx_H]:.3e}')
...
plt.show()
#%% REDEFINO MASAS PARA MATCHEAR Ms 

masa_npm_A_new=masa_npm_A*Ms_A_mean/Ms_R_mean
masa_npm_P_new=masa_npm_P*Ms_P_mean/Ms_R_mean
#%%recalculo todo
campos_A=[]
m_correg_norm_A=[]
fname_A=[]
...
for f in files_A:
    ...
    contrib_diamag_A = chi_mass_laurico*masa_laurico_A*H
    m_correg = (m-contrib_diamag_A)
    m_correg_norm_masa=m_correg/masa_npm_A_new
    ...
    fig,ax=plt.subplots(constrained_layout=True)
    ax.plot(H,m/masa_npm_A_new,'.-',label='A')
    ax.plot(H,m_correg_norm_masa,'.-',label='A (s/ laurico)')
    ax.legend(ncol=2,loc='lower right')
    ax.grid()
    ax.set_xlabel('H (G)')
...
for f in files_P:
    ...
    contrib_diamag_P = chi_mass_laurico*masa_laurico_P*H
    m_correg = (m-contrib_diamag_P)
    m_correg_norm_masa=m_correg/masa_npm_P_new
    ...
    fig,ax=plt.subplots(constrained_layout=True)
    ax.plot(H,m/masa_npm_P_new,'.-',label='P')
    ax.plot(H,m_correg_norm_masa,'.-',label='P (s/ laurico)')
    ax.legend(ncol=2,loc='lower right')
    ax.grid()
    ax.set_xlabel('H (G)')
